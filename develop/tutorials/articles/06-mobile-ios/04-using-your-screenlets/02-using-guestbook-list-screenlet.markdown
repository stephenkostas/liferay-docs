...
1.  In your storyboard, first select the guestbooks scene's view controller. 
    Then drag and drop a plain view (`UIView`) from the Object Library onto the 
...
3.  With the new view selected, open the Identity inspector and set the view's 
    Custom Class to `GuestbookListScreenlet`. The view now appears as Guestbook 
...
class) must conform to the `GuestbookListScreenletDelegate` protocol. Recall 
that you created this delegate when creating the Screenlet. This delegate 
defines methods for responding to the success or failure to retrieve guestbooks, 
and the selection of a guestbook in the list. 
...
3.  Implement the `GuestbookListScreenletDelegate` method 
    ...
    respond to a failed Screenlet call. Its arguments include the resulting 
    `NSError` object. You don't have to do anything in this method, but it's a 
    good idea to print the error:
...
4.  Implement the `GuestbookListScreenletDelegate` method 
    ...
    the segue to the entries scene by using the 
    `performSegue(withIdentifier:sender:)` method with the segue's ID and the 
    selected `GuestbookModel`. Recall that you assigned the segue's ID, 
    `entriessegue`, when you created the segue. Including the selected 
    `GuestbookModel` lets Entry List Screenlet know which guestbook to display 
    entries from: 
